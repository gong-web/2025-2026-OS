...
Shell 是用户与操作系统交互的命令行界面程序。在 ucore 中，Shell 终端作为一个用户态程序运行，当系统启动后会显示：
...
用户可以在 `$` 提示符后输入命令（如 `ls`、`hello` 等），Shell 会解析并执行这些命令。
...
Shell 的主函数实现了经典的 REPL（Read-Eval-Print Loop）模式：
...
**设计要点**：
- 使用 `fork()` 在子进程中执行命令，保持 Shell 主进程不受影响
- 父进程通过 `waitpid()` 等待子进程结束，实现同步执行
- 子进程执行完毕后调用 `exit()`，父进程继续循环

#### `readline()` 函数 - 读取用户输入

该函数负责从标准输入读取一行命令，并实现基本的行编辑功能：
...
**功能特性**：
- 字符回显：用户输入的字符会立即显示在屏幕上
- 退格支持：按下 Backspace 键可以删除字符
- 换行检测：按下 Enter 键结束输入
- Ctrl+C 支持：可以中断当前输入

#### `gettoken()` 函数 - 词法分析

该函数负责将输入的命令行字符串分解成单词（token）：
...
第2次调用: token='w', *p2="-l"
第3次调用: token='>', *p2=">"
第4次调用: token='w', *p2="output.txt"
```

#### `runcmd()` 函数 - 命令执行核心

这是 Shell 最复杂的函数，负责解析和执行命令，支持多种 Shell 特性：

```c
int runcmd(char *cmd) {
    static const char *argv[EXEC_MAX_ARG_NUM + 1];
    int argc = 0;
    ...
    while (1) {
        ...
        // (2) 输入重定向: command < file
        case '<':
            if (gettoken(&cmd, &t) != 'w') {
                printf("sh error: syntax error\n");
                return -1;
            }
            reopen(0, t, O_RDONLY);  // 重定向 stdin
            break;
            
        // (3) 输出重定向: command > file
        case '>':
            if (gettoken(&cmd, &t) != 'w') {
                printf("sh error: syntax error\n");
                return -1;
            }
            reopen(1, t, O_RDWR | O_TRUNC | O_CREAT);  // 重定向 stdout
            break;
            
        // (4) 顺序执行: command1 ; command2
        case ';':
            if ((ret = fork()) == 0) {
                goto runit;  // 子进程执行当前命令
            }
            else {
                waitpid(ret, NULL);  // 等待执行完
                goto again;          // 继续解析下一个命令
            }
            break;
...
**执行流程**：
1. `runcmd("ls")` 解析出 `argv = ["ls", NULL]`
2. `__exec("/ls", argv)` 调用系统调用
3. 内核 `do_execve()` → `load_icode()` 加载 `/ls` 程序
4. 程序执行并输出结果
...
// 1. 解析命令: "ls" ">" "file.txt"
// 2. 执行重定向
reopen(1, "file.txt", O_RDWR | O_CREAT);
// 等价于:
close(1);                              // 关闭 stdout
fd = open("file.txt", O_RDWR|O_CREAT); // 打开文件，返回 fd=1
```
...
之后 `ls` 程序的所有 `printf()` 输出都会写入 `file.txt`。
...
```c
reopen(0, "input.txt", O_RDONLY);
// close(0) → open("input.txt") → dup2(..., 0)
```
...
case ';':
    fork();           // 创建子进程执行 hello
    waitpid(...);     // 等待 hello 执行完毕
    goto again;       // 继续解析并执行 ls
```
...
```c
if (strcmp(argv[0], "cd") == 0) {
    strcpy(shcwd, argv[1]);  // 保存目录路径
    return 0;                 // 不调用 exec
}
```

**注意**：`cd` 必须是内置命令，因为它需要改变 Shell 进程自己的工作目录，不能在子进程中执行。

### 5.5 与文件系统的交互

Shell 的执行涉及到本实验实现的文件系统功能：
...
__exec("ls", argv)
    → sys_exec()             [系统调用]
    → do_execve()
    → sysfile_open("/ls")    [打开磁盘上的程序文件]
    → load_icode()           [练习2 实现的函数]
        → sysfile_read()     [读取 ELF 文件内容]
        → sfs_read()
        → sfs_io_nolock()    [练习1 实现的函数]
```

**关键点**：
- 每次执行命令都会调用 `load_icode()` 从文件系统加载程序
- 文件重定向功能依赖文件系统的 `open()`、`close()`、`dup2()` 实现
- Shell 本身就是一个用户态程序，通过系统调用与内核交互
...
**作用**：确保文件在指定的文件描述符上打开（通常用于 0/1/2）。
...
**用途**：在执行命令前检查文件是否存在，如果不存在则尝试在根目录查找（如将 `ls` 转换为 `/ls`）。

### 5.8 Shell 实现总结

Shell 终端是操作系统中用户交互的关键组件，本实验中的实现虽然简化，但包含了：

| 特性 | 实现状态 | 说明 |
|-----|---------|------|
| **命令执行** | ✅ 完整实现 | 通过 fork + exec 机制 |
| **I/O 重定向** | ✅ 完整实现 | 支持 `<` 和 `>` |
| **顺序执行** | ✅ 完整实现 | 支持 `;` 分隔符 |
| **内置命令** | ✅ 部分实现 | 仅支持 `cd` |
| **管道** | ❌ 未实现 | 代码中已注释掉 `pipe()` 调用 |
| **后台执行** | ❌ 未实现 | 不支持 `&` |
| **通配符** | ❌ 未实现 | 不支持 `*`、`?` 等 |

**核心思想**：
- Shell 作为一个普通用户进程运行
- 通过 `fork()` 创建子进程执行命令，避免影响 Shell 自身
- 利用文件描述符继承机制实现 I/O 重定向
- 所有程序加载都依赖文件系统，体现了操作系统各模块的协同工作
