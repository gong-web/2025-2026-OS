...
sfs_io_nolock(struct sfs_fs *sfs, struct sfs_inode *sin, void *buf, off_t offset, size_t *alenp, bool write) {
  ...
  //LAB8:EXERCISE1 YOUR CODE 2312325 HINT: call sfs_bmap_load_nolock, sfs_rbuf, sfs_rblock,etc. read different kind of blocks in file
	/*
	...
	*/
    if ((blkoff = offset % SFS_BLKSIZE) != 0) {
        ...
        if ((ret = sfs_buf_op(sfs, buf8, size, ino, blkoff)) != 0) {
            goto out;
        }
        alen += size;
        if (nblks == 0) {
    ...
    while (nblks != 0) {
        ...
        if ((ret = sfs_block_op(sfs, buf8, ino, 1)) != 0) {
            goto out;
        }
        alen += size, buf8 += size, blkno ++, nblks --;
    }
    ...
    if ((size = endpos % SFS_BLKSIZE) != 0) {
        ...
        if ((ret = sfs_buf_op(sfs, buf8, size, ino, 0)) != 0) {
            goto out;
        }
        alen += size;
    }
